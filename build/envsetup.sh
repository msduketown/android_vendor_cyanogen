function __print_cyanogen_functions_help() {
cat <<EOF
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            lunch cyanogen_$target-$variant
        fi
    fi
    return $?
}
...
function eat()
{
    if [ "$OUT" ] ; then
        ...
        if (adb shell getprop org.pixelexperience.device | grep -q "$CYANOGEN_BUILD"); then
...
EOF
            ...
            echo "The connected device does not appear to be $CYANOGEN_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function installboot()
{
    ...
    if (adb shell getprop org.pixelexperience.device | grep -q "$CYANOGEN_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell getprop org.pixelexperience.device | grep -q "$CYANOGEN_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function repopick() {
    T=$(gettop)
    $T/vendor/cyanogen/build/tools/repopick.py $@
}
